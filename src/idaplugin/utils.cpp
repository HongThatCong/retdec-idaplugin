...
ida_version_t getIDAVersion()
{
    ...
    ssize_t ret = get_kernel_version(szVer, sizeof(szVer));
    if (ret <= 0)
        return ver_error;
...
{
    plugin_info_t *plgs = get_plugins();
    VERIFY(nullptr != plgs);
    if (nullptr == plgs)
    {
        return "";
    ...
    while (nullptr != plgs)
    {
        if (RetDec::pluginName == plgs->name)
        {
            std::filesystem::path p = plgs->path;
            p.remove_filename();
            p = p.parent_path();    // plugins directory
            p = p.parent_path();    // ida or ida user dir directory
            return p.string();
        }
